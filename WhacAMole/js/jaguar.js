...
collie.LayerEvent = collie.Class(/** @lends collie.LayerEvent.prototype */{
    ...
    _onEvent : function (e) {
        ...
        e = e || window.event;
        if (this._bHasTouchEvent) {
          // console.log('event type:'+ e.type +'count:'+e.changedTouches.length);
          for (var i=0; i<e.changedTouches.length; i++) {
            this._onSingleEvent(e, e.changedTouches[i]);
          }
        }
        else {
          this._onSingleEvent(e, e);
        }
    },
     ...
     * @param {HTMLEvent} e
     */
    _onSingleEvent : function (e, oEvent) {
        // 이벤트를 사용하지 않으면 무시
        ...
        if (sType === "touchcancel") {
            if (this._htEventStartPos !== null) {
                nRelatedX = this._htEventStartPos['touchid_'+oEvent.identifier].x;
                nRelatedY = this._htEventStartPos['touchid_'+oEvent.identifier].y;
            }
        }
        ...
        if (sType === "mousedown") {
          ...
          if (this._bHasTouchEvent) {
            if (!this._htEventStartPos) {
              this._htEventStartPos = {};
            }
            this._htEventStartPos['touchid_'+oEvent.identifier] =  currStartPos;
          }
          else {
            this._htEventStartPos = currStartPos;
          }
        }
        ...
        if (sType === "mouseup") {
            ...
            var currStartPos = null;
            if (this._bHasTouchEvent) {
              currStartPos = this._htEventStartPos['touchid_'+oEvent.identifier];
            }
            else {
              currStartPos = this._htEventStartPos;
            }
            // console.log(currStartPos);

            if (
                currStartPos &&
                currStartPos.x - nThresholdX <= nRelatedX &&
                nRelatedX <= currStartPos.x + nThresholdX &&
                currStartPos.y - nThresholdY <= nRelatedY &&
                nRelatedY <= currStartPos.y + nThresholdY
                ) {
                // console.log('fire click');
                this._fireEvent(e, "click", nRelatedX, nRelatedY);
            }
            if (this._bHasTouchEvent) {
              delete this._htEventStartPos['touchid_'+oEvent.identifier];
            }
            else {
    ...
    _unsetMousedownObject : function (oDisplayObject) {
        if (this._oMousedownObject === oDisplayObject) {
            if (this._oMousedownObjectCount <= 1) {
              this._oMousedownObject = null;
              this._oMousedownObjectCount=0;
            }
            else {
              this._oMousedownObjectCount--;
            }
...
collie.Renderer = collie.Renderer || new (collie.Class(/** @lends collie.Renderer */{
     ...
     */
    _setEventStatusCount: 0,
    setEventStatus : function (sEventType, bFiredOnTarget) {
        if (this._htEventStatus.type == sEventType && this._htEventStatus.firedOnTarget == bFiredOnTarget) {
          this._setEventStatusCount += 1;
        }
        else {
          this._setEventStatusCount = 1;
          this._htEventStatus = {
    ...
    isStopEvent : function (sEventType) {
        ...
        var isEq = sEventType === this._htEventStatus.type && this._htEventStatus.firedOnTarget;
        if (this._setEventStatusCount<=1) {
          this._setEventStatusCount = 0;
          return isEq
        }
        else {
          this._setEventStatusCount--;
          return false
        }
